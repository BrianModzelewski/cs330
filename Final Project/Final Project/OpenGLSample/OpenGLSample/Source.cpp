...
namespace
{
	...
	// camera
	Camera gCamera(glm::vec3(-3.0f, 3.0f, 8.0f));
...
int main(int argc, char* argv[])
{
	...
	// Load texture
	texFilename = "black.png";
	if (!UCreateTexture(texFilename, gBlackTextureId))
	{
	...
	if (!UCreateTexture(texFilename, gWoodTextureId))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
	...
	// Load texture
	texFilename = "screen.png";
	if (!UCreateTexture(texFilename, gScreenTextureId))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
	...
	if (!UCreateTexture(texFilename, gKeyboardTextureId))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
	...
	glBindTexture(GL_TEXTURE_2D, gBlackTextureId);
	...
	glUniform2f(glGetUniformLocation(gSurfaceProgramId, "uvScale"), 1.0, 1.0);
	...
	UDestroyTexture(gBlackTextureId);
...
void UProcessInput(GLFWwindow* window)
{
	...
	// Handle QE keys for upward and downward movement
	...
	//Handle PO keys for Perspective/ Ortho views
	if (glfwGetKey(window, GLFW_KEY_P) == GLFW_PRESS);
	isOrthographic = false; // Switch to perspective projection

	if (glfwGetKey(window, GLFW_KEY_O) == GLFW_PRESS)
		isOrthographic = true; // Switch to orthographic projection
...
void URender()
{
	...
	glUniform3f(light1ColLoc, 0.8f, 0.8f, 1.1f);
	glUniform3f(light1PosLoc, -3.5f, 6.0f, 0.0f);
	glUniform3f(light2ColLoc, 0.8f, 0.8f, 1.1f);
	glUniform3f(light2PosLoc, -4.0f, 3.0f, 12.0f);
	//set specular intensity
	glUniform1f(specInt1Loc, 1.6f);
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(16.0f, 0.2f, 6.0f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 0.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-0.15f, -0.11f, 0.0f));
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(5.5f, 0.2f, 9.0f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 0.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(5.1f, -0.11f, 7.5f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(2.0f, 5.0f, 6.0f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 0.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-7.15f, -2.71f, -0.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(5.5f, 5.0f, 2.0f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 0.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(5.1f, -2.71f, 11.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(3.0f, 5.0f, 0.25f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 0.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(6.35f, -2.71f, -2.87f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(0.25f, 5.0f, 3.0f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 0.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(7.72f, -2.71f, -1.25f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(2.4f, 6.0f, 1.4f));
	// 2. Rotate the object
	rotation = glm::rotate(4.71f, glm::vec3(1.0, 0.0f, 0.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-4.5f, 2.0f, -2.32f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// Draws the triangles
	glDrawElements(GL_TRIANGLES, meshes.gPlaneMesh.nIndices, GL_UNSIGNED_INT, (void*)0); //Creates the plane
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(1.4f, 1.0f, 0.5f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-4.5f, 0.102f, 1.5f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(0.4f, 0.1f, 0.8f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3( -1.0f, 0.051f, 2.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(2.8f, 0.1f, 0.98f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-4.5f, 0.051f, 1.5f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(5.0f, 3.0f, 0.25f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-4.5f, 2.0f, -2.45f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-4.5f, 0.0f, -2.5f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(0.1f, 0.1f, 1.5));
	// 2. Rotate the object
	rotation = glm::rotate(75.0f, glm::vec3(0.0, 1.0f, 0.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-4.85f, 0.05f, -2.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	scale = glm::scale(glm::vec3(0.1f, 0.1f, 1.5));
	// 2. Rotate the object
	rotation = glm::rotate(76.0f, glm::vec3(0.0, 1.0f, 0.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(-4.05f, 0.05f, -2.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
